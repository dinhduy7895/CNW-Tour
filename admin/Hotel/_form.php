...
<form action="_check.php<?php echo isset($id) ? '?id=' . $id : ''; ?>" method="post">
    ...
    <div class="form-group field-user-username required">
     ...
     <div class="form-group field-user-username required">
        <label class="control-label" for="user-username" >Level</label>
        <input type="text" id="user-username" class="form-control" name="level" maxlength="255" required="required" value='<?php echo isset($row) ? $row['level'] : ''; ?>'>

        <div class="help-block"></div>
    </div>
    
    <?php include './_formHotelAdress.php'; ?>
